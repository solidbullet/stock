...
for x in mydoc:
    ...
    reback = round(x['reback'],2)
    diff30 = round(x['diff30'],2)
    volumn = round(x['volumn'],2)
    rise = round(x['rise'], 2)
    dict1 = {'id': [stockid], 'name': [stockname], 'diff30':[diff30],'volumn':[volumn],'reback': [reback],'vibration':[vibration]}
    d = pd.DataFrame(dict1)
    dict_df = dict_df.append(d)
# for item in col.find():
dict_df = dict_df.dropna()
reback = dict_df.sort_values(by = 'reback',ascending = False)[0:500]
vibration = dict_df.sort_values(by = 'vibration',ascending = False)[0:500]
